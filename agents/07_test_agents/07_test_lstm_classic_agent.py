...
def setup_logger():
    logger = logging.getLogger('07_test_lstm_classic_agent')
    logger.setLevel(logging.INFO)
    os.makedirs('logs/agents', exist_ok=True)
    fh = logging.FileHandler('logs/agents/07_test_lstm_classic_agent.log')
...
def load_model_and_scaler():
    ...
    model_path = os.path.join(model_dir, 'lstm_classic', 'trained_model.h5')
    scaler_path = os.path.join(model_dir, 'lstm_classic', 'scaler.pkl')
    if not os.path.exists(model_path) or not os.path.exists(scaler_path):
        raise FileNotFoundError("LSTM classic model or scaler not found")
...
def evaluate_and_save(model, scaler, X, y, timestamps, transaction_keys, logger):
    ...
    logger.info(f"LSTM Classic Test - RMSE: {rmse:.3f}, MAE: {mae:.3f}, MAPE: {mape:.2f}%")
    ...
    results_path = os.path.join(out_dir, 'lstm_classic_test_results.csv')
    results.to_csv(results_path, index=False)
    logger.info(f"Saved LSTM classic test results to {results_path}")


def main():
    logger = setup_logger()
    logger.info("Starting LSTM classic testing...")
    ...
    logger.info("LSTM classic testing completed.")
