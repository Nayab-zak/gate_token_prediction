...
def setup_logger():
    logger = logging.getLogger('07_test_lgbm_augmented_agent')
    logger.setLevel(logging.INFO)
    os.makedirs('logs/agents', exist_ok=True)
    fh = logging.FileHandler('logs/agents/07_test_lgbm_augmented_agent.log')
...
def load_model():
    model_path = os.path.join(os.getenv('MODEL_DIR', 'models'), 'lgbm_augmented', 'trained_model.pkl')
...
def evaluate_and_save(model, X, y, timestamps, transaction_keys, logger):
    ...
    logger.info(f"LightGBM Augmented Test - RMSE: {rmse:.3f}, MAE: {mae:.3f}, MAPE: {mape:.2f}%")
    ...
    results_path = os.path.join(out_dir, 'lgbm_augmented_test_results.csv')
    results.to_csv(results_path, index=False)
    logger.info(f"Saved LightGBM augmented test results to {results_path}")


def main():
    logger = setup_logger()
    logger.info("Starting LightGBM augmented testing...")
    model = load_model()
    X, y, timestamps, transaction_keys = load_test_data()
    evaluate_and_save(model, X, y, timestamps, transaction_keys, logger)
    logger.info("LightGBM augmented testing completed.")
