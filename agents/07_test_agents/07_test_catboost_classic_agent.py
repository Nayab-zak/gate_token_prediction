...
def setup_logger():
    logger = logging.getLogger('07_test_catboost_classic_agent')
    logger.setLevel(logging.INFO)
    os.makedirs('logs/agents', exist_ok=True)
    fh = logging.FileHandler('logs/agents/07_test_catboost_classic_agent.log')
...
def load_model():
    model_path = os.path.join(os.getenv('MODEL_DIR', 'models'), 'catboost_classic', 'trained_model.pkl')
...
def evaluate_and_save(model, X, y, timestamps, transaction_keys, logger):
    ...
    logger.info(f"CatBoost Classic Test - RMSE: {rmse:.3f}, MAE: {mae:.3f}, MAPE: {mape:.2f}%")
    ...
    results_path = os.path.join(out_dir, 'catboost_classic_test_results.csv')
    results.to_csv(results_path, index=False)
    logger.info(f"Saved CatBoost classic test results to {results_path}")
    logger.info(f"Results include transaction keys: TerminalID, MoveType, Desig")


def main():
    logger = setup_logger()
    logger.info("Starting CatBoost classic testing...")
    model = load_model()
    X, y, timestamps, transaction_keys = load_test_data()
    evaluate_and_save(model, X, y, timestamps, transaction_keys, logger)
    logger.info("CatBoost classic testing completed.")
