...
def setup_logger():
    logger = logging.getLogger('07_test_rf_classic_agent')
    logger.setLevel(logging.INFO)
    os.makedirs('logs/agents', exist_ok=True)
    fh = logging.FileHandler('logs/agents/07_test_rf_classic_agent.log')
...
def load_model():
    model_path = os.path.join(os.getenv('MODEL_DIR', 'models'), 'rf_classic', 'trained_model.pkl')
...
def evaluate_and_save(model, X, y, timestamps, transaction_keys, logger):
    ...
    logger.info(f"RF Classic Test Results - RMSE: {rmse:.3f}, MAE: {mae:.3f}, MAPE: {mape:.2f}%")
    ...
    results_path = os.path.join(out_dir, 'rf_classic_test_results.csv')
    results.to_csv(results_path, index=False)
    logger.info(f"Saved RF classic test results to {results_path}")


def main():
    logger = setup_logger()
    logger.info("Starting RF classic testing...")
    model = load_model()
    ...
    logger.info("RF classic testing completed.")
