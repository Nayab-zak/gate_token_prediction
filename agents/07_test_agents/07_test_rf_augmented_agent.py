...
def setup_logger():
    logger = logging.getLogger('07_test_rf_augmented_agent')
    logger.setLevel(logging.INFO)
    os.makedirs('logs/agents', exist_ok=True)
    fh = logging.FileHandler('logs/agents/07_test_rf_augmented_agent.log')
...
def load_model():
    model_path = os.path.join(os.getenv('MODEL_DIR', 'models'), 'rf_augmented', 'trained_model.pkl')
...
def load_test_data():
    ...
    X_test = df_encoded.drop(columns=['datetime', 'TokenCount'])
    y_true = df_encoded['TokenCount']
    ...
    return X_test, y_true, timestamps, transaction_keys
...
def evaluate_and_save(model, X, y, timestamps, transaction_keys, logger):
    ...
    logger.info(f"RF Augmented Test Results - RMSE: {rmse:.3f}, MAE: {mae:.3f}, MAPE: {mape:.2f}%")
    ...
    results_path = os.path.join(out_dir, 'rf_augmented_test_results.csv')
    results.to_csv(results_path, index=False)
    logger.info(f"Saved RF augmented test results to {results_path}")


def main():
    logger = setup_logger()
    logger.info("Starting RF augmented testing...")
    model = load_model()
    X_test, y_true, timestamps, transaction_keys = load_test_data()
    evaluate_and_save(model, X_test, y_true, timestamps, transaction_keys, logger)
    logger.info("RF augmented testing completed.")
