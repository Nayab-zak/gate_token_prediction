#!/usr/bin/env python3
"""
XGBoost Training Agent - Dense embeddings
"""
...
class TrainXGBoostAgent(BaseTrainingAgent):
    def __init__(self, config_path: str = "config.yaml", log_dir: str = "logs"):
        super().__init__("xgboost", config_path, log_dir)
    
    def get_search_space(self) -> Dict[str, Any]:
        """Return hyperparameter search space for XGBoost"""
        return self.config['tuning']['search_spaces']['xgboost']
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train XGBoost Agent')
    ...
    agent = TrainXGBoostAgent(args.config)
